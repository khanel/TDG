#!/usr/bin/env python3
"""
Comprehensive Validation Runner for Effectiveness-First Reward (EFR)
...
Usage:
    python validate_efr.py                    # Run all tests
    python validate_efr.py --scenarios-only   # Only behavioral tests
    python validate_efr.py --math-only        # Only mathematical tests
"""
...
def run_behavioral_validation(reward_computer: RewardWrapper, scenarios: List[Dict]) -> ValidationResult:
    """
    ...
    for scenario in scenarios:
        ...
        try:
            signal = reward_computer.compute(inputs, action, info)
            val = signal.value
...
def run_smoothness_analysis(reward_computer: RewardWrapper) -> ValidationResult:
    """
    ...
    info = {'current_phase': 'EXPLORATION'}
    
    for stag in np.linspace(0, 1, 100):
        inputs['stagnation_ratio'] = stag
        signal = reward_computer.compute(inputs, 0, info)
        val = signal.value
    ...
    info = {'current_phase': 'EXPLOITATION'}
    ...
    inputs = {
        "fitness_norm": 0.5,
        "improvement_velocity": 0.8,  # With high improvement velocity
...
def run_lipschitz_analysis(reward_computer: RewardWrapper, num_samples: int = 2000) -> ValidationResult:
    """
    ...
    for phase in ['EXPLORATION', 'EXPLOITATION', 'TERMINATION']:
        ...
        for action in actions:
            ...
            for _ in range(num_samples):
                ...
                # Get rewards
                r1 = reward_computer.compute(base_state, action, info).value
                r2 = reward_computer.compute(perturbed_state, action, info).value
...
def run_boundedness_check(reward_computer: RewardWrapper, num_samples: int = 5000) -> ValidationResult:
    """
    ...
    for phase in ['EXPLORATION', 'EXPLOITATION', 'TERMINATION']:
        ...
        for action in actions:
            for _ in range(num_samples):
                ...
                signal = reward_computer.compute(state, action, info)
                val = signal.value
...
def main():
    parser = argparse.ArgumentParser(description="Validate Effectiveness-First Reward (EFR) Function")
    ...
    # Initialize reward computer with EFR
    reward_computer = RewardWrapper(EFRConfig())
    ...
    if not args.math_only:
        ...
        try:
            ...
            filtered_scenarios = [s for s in scenarios.get('single_state_scenarios', []) if '_section' not in s]
            results['Behavioral Scenarios'] = run_behavioral_validation(reward_computer, filtered_scenarios)
