...
[x] **3.1 Minimal Observation Definition:** Define the input vector for the policy.
    - *Initial proposal: A 6-dimensional vector including metrics like (time_since_last_improvement, fraction_of_budget_used, etc.).*
[x] **3.2 Observation Computer:** Design a component to compute the observation vector from the core system state.
...
[x] **4.1 Minimal Reward Definition:** Design a reward signal to encourage efficient stage transitions.
[x] **4.2 Reward Computer:** Implement the logic for calculating the reward.
...
[x] **7.1 Adapter Contract:** Define how a specific problem (like TSP) provides solvers and configuration for the environment.
[x] **7.2 Initial TSP Wiring:** Create an adapter for the Traveling Salesperson Problem as the first use case.
...
[ ] **9.1 Refactoring Plan:**
    - `RLOrchestrator/core/orchestrator.py`: Has been extended to include the `OrchestratorEnv` logic.
    - `RLOrchestrator/core/observation.py`: Has been extended to include the `ObservationComputer`.
    - `RLOrchestrator/core/reward.py`: Has been extended to include the `RewardComputer`.
    - `RLOrchestrator/tsp/env.py`: New file containing the TSP-specific wiring for the environment has been created.
