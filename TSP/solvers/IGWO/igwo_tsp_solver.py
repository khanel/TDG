...
class TSPIGWOSolver(IGWO):
    def __init__(self, graph, city_coords, population_size, dim, lower_bound, upper_bound, num_iterations=100):
        ...
        super().__init__(
            objective_func=self.tsp_fitness,
            dim=dim,
            lb=lower_bound,
            ub=upper_bound,
    ...
    def tsp_fitness(self, perm):
        # Ensure perm is a valid permutation (1-based city indices)
        perm = np.array(perm).astype(int)
        if perm[0] != 1:
            perm = np.insert(perm, 0, 1)
        return self.calculate_path_distance(perm)

    def calculate_path_distance(self, route):
        # Calculate total distance for the given route
        dist = 0.0
        for i in range(len(route)):
            city1 = route[i] - 1
            city2 = route[(i + 1) % len(route)] - 1
            dist += self.graph.get_weights()[city1][city2]
        return dist

    def initialize_population(self):
        # Each individual is a permutation of cities (starting with city 1)
        population = []
        ...
        # No lens imaging reverse for discrete TSP; fallback to pure permutations
