...
def main():
    ...
    logger.info(
        f"Run: mode=eval, session_id={session_id}, problem=tsp, episodes={int(args.episodes)}"
    )
    logger.info(
    ...
    for episode_idx in range(1, max(1, args.episodes) + 1):
        ...
        while not done:
            ...
            # Optional step debug (kept at DEBUG level)
            logger.debug(
                f"Step {step_idx}: phase_before={phase_before}, action={int(action)}, phase_after={phase_after}, "
                f"reward={float(reward):.3f}, terminated={bool(terminated)}, truncated={bool(truncated)}, "
                f"best={(float(candidate.fitness) if candidate and candidate.fitness is not None else None):.3f}, "
                f"improvement={(float(improvement) if improvement is not None else None):.3f}"
            )
